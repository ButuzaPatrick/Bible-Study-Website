...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Event listener for click to toggle highlighting
  chapterContainer.addEventListener("click", function(event) {
    const verse = event.target.closest(".verse-wrapper");
    if (verse) {
  ...
  chapterContainer.addEventListener("mouseover", function(event) {
    const verse = event.target.closest(".verse-wrapper");
    if (verse) {
      verse.classList.add("hovered"); // Add underline
      const index = Array.from(verse.parentNode.children).indexOf(verse);
      tooltip.innerText = `Verse: ${index + 1}`;
  ...
  chapterContainer.addEventListener("mouseout", function(event) {
    const verse = event.target.closest(".verse-wrapper");
    if (verse) {
      verse.classList.remove("hovered"); // Remove underline
...
function toggleVerseHighlight(verse) {
  // Toggle background color for highlighting
  if (verse.style.backgroundColor === 'black') {
    ...
    openMainMenu();
    verse.style.backgroundColor = 'black'; // Set background color
